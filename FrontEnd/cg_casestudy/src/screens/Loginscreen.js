...
export default function Loginscreen() {
    ...
    async function login(){
      ...
      try {
        setloading(true)
        const result = await axios.post('http://localhost:8682/auth',user)
        if(await result.data.response !== "Authentication successful") throw new Error();
        localStorage.setItem('currentUser',JSON.stringify(JSON.parse(await result.config.data).username));
        window.location.href='/'
      } catch (error) {
        seterror(true)
        setloading(false)
